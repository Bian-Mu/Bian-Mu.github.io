...
1. 组件：常通过函数组件+Hooks
2. JSX：类似于HTML的语法糖，最终被createElement调用
3. 虚拟DOM：在内存中树状存储，由diff批量更新后渲染成真实DOM
4. Hooks：实现功能的灵活方案

diff在内存中对比新旧树，找出需要更新的最小部分，对于子树，采用启发式：

1. div->span：直接更新
2. 同类节点只更新props与children
3. 同层节点按照key来比较

fiber架构将渲染拆分，允许中断、按照优先级调度、增量渲染
